...
func (tree *AVL) set(key string, secondaryKey string, value string) string {
	...
	response := tree.changeHeights(tree.secondaryRoot, secondaryNode.parent)
	if response != "ok" {
		return response
	}
